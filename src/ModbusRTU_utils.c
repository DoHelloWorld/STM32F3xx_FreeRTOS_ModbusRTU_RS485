...
const uint8_t MB_CRC_Table_Lo[] = {     0x00, 0xC0, 0xC1, 0x01, 0xC3, 0x03, 0x02, 0xC2, 0xC6, 0x06, 0x07, 0xC7, 0x05, 0xC5, 0xC4,
                                        ...
                                        0x40    };
#else
static void MB_CRC_Init(void);

static void MB_CRC_Init(void)
{
    const uint16_t MB_POLY = 0x8005;
    RCC->AHBENR |= RCC_AHBENR_CRCEN;
    CRC->CR &= ~(CRC_CR_POLSIZE | CRC_CR_REV_IN);
    CRC->CR |= (CRC_CR_POLSIZE_0 | CRC_CR_REV_OUT | CRC_CR_REV_IN_0);
    CRC->POL = MB_POLY;
    CRC->CR |= CRC_CR_RESET;
}
#endif
...
{
    return (address == MB_SLAVE_ADDRESS) ? 1 : 0;
}
...
{
    return (receivedCRC == calculatedCRC) ? 1 : 0;
}
